...
bool OperatorNodeTwoNodes::addNext(TreeParser &parser)
{
    left = parser.getToken();
    if (left == nullptr)
        return false;

    if(!left->addNext(parser))
    {
        delete left;
        left = nullptr;
        return false;
...
bool OperatorNodeOneNode::addNext(TreeParser &parser)
{
    child = parser.getToken();
    if (child == nullptr)
        return false;

    if (!child->addNext(parser))
    {
        delete child;
        child = nullptr;
        return false;
...
bool ValueNode::addNext(TreeParser &parser)
{
    return true;
}
